...
const deckShadow = (pos, cards) => [...Array(cards / 2 | 0).keys()]
.map(i => {
...
function flipCard(deck) {
  ...
  setTimeout(() => {
    ...
    movingCard.style.left = x - (vert? width/2.02 : width/80) + 'px'
    movingCard.style.top = y - (vert? height/80 : height/2.02) + 'px'
